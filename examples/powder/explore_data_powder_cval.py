...
def main():
    ...
    ddicts = sorted(get_data_dicts(json_path), key=lambda f: f['file_name'])
    rs = np.random.RandomState(98)
    rs.shuffle(ddicts)

    train_splits = []
    test_splits = []
    if EXPERIMENT_NAME == 'particles':
    ...
    elif EXPERIMENT_NAME== 'satellites':
        for i in range(len(ddicts)//2):
            train = ddicts[0:2*i]
            test = ddicts[2*i:2*i+2]
            train += ddicts[2*i+2:]
            train_splits.append(train)
            test_splits.append(test)

    else:
        raise ValueError('dataset must be particles or satellites')

    OUTPUT_ROOT = pathlib.Path('./output/')
    for i, (trainset, testset) in enumerate(zip(train, test)):
        ...
        for key, value in zip(['Training','Validation'], [trainset, testset]):
            name = EXPERIMENT_NAME + '_' + key + 'cval_{}'.format(i)
            DatasetCatalog.register(name, lambda _=key: value)
