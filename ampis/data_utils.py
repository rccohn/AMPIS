...
def get_ddicts(label_fmt, im_root, ann_root=None, pattern='*', dataset_class=None):
    ...
    im_root: str, Path object.
        ...
        If label == 'rle', im_root is the path to the pickle file containing a list of dictionaries of form
        { 'file_name' : <relative_path_to_image>, 'segmentations':[<rle_masks>]}.Image directory should be in the 
        same path as im_root
    ...
    if label_fmt.lower() in ['binary', 'label']:
        ...
        im_root=Path(im_root)
        masks=pickle.load(open(im_root,'rb'))
