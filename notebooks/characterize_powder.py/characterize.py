...
from . import analyze, structures
...
def _rle_satellite_match(particles, satellites, match_thresh=0.5):
    ...
    particles = structures.masks_to_rle(particles)
    satellites = structures.masks_to_rle(satellites)

    satellite_matches = []
    intersection_scores = []

    particles_matched_bool = np.zeros(lenparticles, dtype=np.bool)
    ...
    particles_unmatched = np.array([i for i, x in enumerate(particles_matched_bool) if not x], np.int)
...
class powder_satellite_image(object):
    ...
    def compute_matches(self):
        ...
        """
        self.matches = fast_satellite_match(self.particles.instances.masks, self.satellites.instances.masks,
                                            self.particles.instances.boxes, self.satellites.instances.boxes)
