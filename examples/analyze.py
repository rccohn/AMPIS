...
class instance_set(object):
    ...
    def read_from_ddict(self, ddict, return_=False):
        ...
        instances = Instances(image_size, **{'masks': instances_gt.gt_masks,
                                             'boxes': instances_gt.gt_boxes,
                                             'class_idx': instances_gt.gt_classes})
        self.instances = instances
...
def quick_display_instances(img, metadata, iset, show_class_idx=False, show_scores=False,  ax=None, colors=None):
    ...
    vis = V.overlay_instances(boxes=iset.instances.boxes, masks=iset.instances.masks, labels=labels,
...
def filter_mask_size(instances, min_thresh=100, max_thresh=100000,):
    """
    ...
    inliers = np.logical_and(inlier_min, inlier_max)

    inlier_idx = list(np.arange(len(masks), dtype=np.int)[inliers])

    instances_filtered = instances[inlier_idx]
