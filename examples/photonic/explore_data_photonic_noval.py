...
def get_data_dicts(data_root):
    png_files = sorted([x for x in data_root.glob('*.png') if 'label' not in x.name])
...
def main():
    ...
    data_root = pathlib.Path('../../data/raw/photonic/noval/train/')
    ...
    dataset_name = 'photonic_Training'
    DatasetCatalog.register(dataset_name, lambda x=data_root: get_data_dicts(data_root))
    MetadataCatalog.get(dataset_name).set(**get_metadata())  # labels removed because they crowd images.
    ...
    cfg.DATASETS.TRAIN = (dataset_name,)  # name of training dataset (must be registered)
    cfg.DATASETS.TEST = ()  # name of test dataset (must be registered)
    ...
    cfg.OUTPUT_DIR = str(pathlib.Path(OUTPUT_ROOT))
    ...
    test_images = sorted(pathlib.Path('..','..','data','raw','photonic','noval','test').glob('*.png'))
    for p in checkpoint_paths:
        ...
        for path in test_images:
            img = cv2.imread(str(img_path))
            out = predictor(img)
            outputs[path.name] = data_utils.format_outputs(path.name, 'test', out)
            visualize.quick_visualize_ddicts(out['instances'],outdir,dataset,gt=False,img_path=path)
